    ...
    def recipient_stress(self):
        ...
        if self.recipient == Person.BOSS:
            ...
            sender_multiplier = 0.005 if self.sender == Person.EMPLOYEE else \
                                ...
                                0.25 if self.sender == Person.BOSS else 0
...
class Employee(mesa.Agent):
    ...
    def calculate_stress(self):
        ...
        if self.model.work_hours == Hours.AFTER:
            if recent_messages < 8:
                reduce_stress = 0.05 * logistic_factor
                stress += -(reduce_stress)
            elif recent_messages < 64:
...
def model_running(num_nodes, hours_to_run, after_hours_messaging, graph=None):
    if after_hours_messaging == 0:
        model = EmployeeCommunication(num_nodes=num_nodes, messaging_after_hours=False, graph=graph)
    else:
        model = EmployeeCommunication(num_nodes=num_nodes, after_hours_comm_frequency=after_hours_messaging, graph=graph)
    model.run_model(hours_to_run) # 8760 hours in a year

    print(f"The average stress from this run is {model.avg_stress():.2f}.")

    fig,ax=plt.subplots(1,1,figsize=(8,8))
    model.plot_grid(fig, layout='spring', title='Employee ICT Messaging', close=False)
    plt.close()
    ...
    plt.savefig("plots/employee/average_stress.png")
    plt.close()
    ...
    plt.savefig("plots/employee/stress_per_worker.png")
    plt.close()
    ...
    for line in legend.get_lines():
        line.set_linewidth(4)
    plt.savefig("plots/employee/averagee_stress_by_stress_type.png")
    plt.close()
